...
public enum AgentDetector {
    ...
    public static func detectRunningAgents() async -> [DetectedAgent] {
        return await Task.detached(priority: .utility) {
            ...
            let processList = await getProcessListAsync()
                ...
                for process in processList {
                    if matchesPattern(process, pattern: pattern) {
                        let agent = DetectedAgent(
                            ...
                            type: pattern.type,
                            pid: process.pid,
                            terminalApp: await detectTerminalAppAsync(for: process),
                            windowTitle: nil
                        )
                        // Avoid duplicates
    ...
    /// Get list of processes asynchronously using concurrent task
    private static func getProcessListAsync() async -> [ProcessInfo] {
        return await Task.detached(priority: .utility) {
    ...
    /// Detect which terminal app a process is running in (async version)
    private static func detectTerminalAppAsync(for process: ProcessInfo) async -> String? {
        return await Task.detached(priority: .utility) {
