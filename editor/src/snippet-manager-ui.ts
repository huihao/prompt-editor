...
export class SnippetManagerUI {
  ...
  private async saveCategory(): Promise<void> {
    console.log('[SnippetManagerUI] saveCategory called');
    
    const id = (document.getElementById('category-id') as HTMLInputElement)?.value.trim();
    const name = (document.getElementById('category-name') as HTMLInputElement)?.value.trim();
    const icon = (document.getElementById('category-icon') as HTMLInputElement)?.value.trim() || '📁';
    const description = (document.getElementById('category-desc') as HTMLInputElement)?.value.trim();
    ...
    console.log('[SnippetManagerUI] Form values:', { id, name, icon, description, parentId });
    ...
    const isEditing = this.editingItem !== null;
    console.log('[SnippetManagerUI] isEditing:', isEditing);
    ...
    try {
      if (isEditing) {
        ...
        console.log('[SnippetManagerUI] Calling addCategory with:', category, parentId);
        success = await snippetManager.addCategory(category, parentId || undefined);
        console.log('[SnippetManagerUI] addCategory result:', success);
      }
      ...
      console.error('[SnippetManagerUI] Error saving category:', error);
